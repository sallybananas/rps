// Initialize Firebase
var config = {
    apiKey: "AIzaSyAMGvi_mIMFmd2aUN6D2Cc5_YDVe3wvH6E",
    authDomain: "rockpaperscissor-e7395.firebaseapp.com",
    databaseURL: "https://rockpaperscissor-e7395.firebaseio.com",
    projectId: "rockpaperscissor-e7395",
    storageBucket: "rockpaperscissor-e7395.appspot.com",
    messagingSenderId: "488390657393"
  };
...
$(document).on('ready', function() {
	...
	var game = {
		listeners: function() {
			...
			// Start button click after name entered
			$('#addName').one('click',function() {
				game.setPlayer();
				return false;
			});
			// Show player name in box after pressing button
			playersRef.on('child_added', function(childSnapshot) {
				// Gets a unique player number
				var key = childSnapshot.key();
				// Gets a unique player names
				name[key] = childSnapshot.val().name;
				// Remove loading image and add player name to game box
				var waiting = $('.player' + key + ' > .waiting');
				waiting.empty();
				var $h1 = $('<h1>').text(name[key]);
				waiting.append($h1);				
				// Get player wins and losses and display in game box
		...
		addPlayer: function(count) {
			// Gets player name that was entered
		...
		showChoice: function() {
			...
			// Listen for choice of item to play rock paper or scissors
		...
		setChoice: function() {
			...
			// Clear choices and add choice to game box
			var $i = $('<i>');
